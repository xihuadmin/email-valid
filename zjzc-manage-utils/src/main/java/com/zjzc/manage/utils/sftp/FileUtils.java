...
public class FileUtils extends org.apache.commons.io.FileUtils {
	 ...
	 * @param fileName 要写入的文件
	 */
	public static void writeToFile(String path,String fileName, String content, String encoding, boolean append) {
		try {
			File f=new File(path);
			if  (!f .exists()  && !f.isDirectory())//判断是否存在path文件夹，没有就创建文件夹
				f.mkdir(); 
	 ...
	 * @param fileName 要写入的文件
	 */
	public static void writeToFile(String path,String fileName, String content, String encoding) {
		try {
			File f=new File(path);
			if  (!f .exists()  && !f.isDirectory())//判断是否存在path文件夹，没有就创建文件夹
				f.mkdir(); 
			fileName=path+fileName;
			FileUtils.write(new File(fileName), content, encoding, true);
			//log.debug("文件 " + fileName + " 写入成功!");
		} catch (IOException e) {
			log.debug("文件 " + fileName + " 写入失败! " + e.getMessage());
		}
	...
	public static void downLoadFile(String filePath, HttpServletResponse response, String fileName)
	        ...
	        response.setHeader("Content-Disposition", "attachment;filename="
					.concat(new String(fileName.getBytes("UTF-8"), "ISO8859-1")));
